    ...
    if (control) {
        switch (move) { // Player is 08 / 12
            case 0:
                ...
                break;
            case 1:
                if (map[ypos + 8][xpos + 11] != undefined) {﻿// Get canvas
var canvas = document.getElementById("canvas");
if (canvas.getContext) {
    var ctx = canvas.getContext("2d");
}
...
function loadsavebutton(x) {
    ...
    ctx.font = "12px NotoSans, sans-serif";
    ctx.fillStyle = "rgb(209, 167, 199)";

    if (localStorage["SOTR" + (x + 1)] != null && localStorage["SOTR" + (x + 1)] != "null") {
        ctx.fillText("Save " + (x + 1), 80 + (x * 100), 40);
        ctx.fillStyle = "rgb(200, 220, 0)";
        ctx.fillText("Level " + (JSON.parse(localStorage["SOTR" + (x+1)]).characters.bleu.level), 80 + (x *  200), 55);
    }
...
function deletesavebutton(x) {
    ctx.fillStyle = "rgb(90, 10, 74)";
    ctx.fillRect(35 + (x * 100), 112, 96, 58);
    ctx.fillStyle = "rgb(160, 40, 167)";
    ctx.fillRect(40 + (x * 100), 117, 86, 48);

    ctx.drawImage(images.gear, 40 + (x * 100), 127, 30, 30);
    ctx.font = "12px NotoSans, sans-serif";
    ctx.fillStyle = "rgb(229, 127, 159)";
    if (localStorage["SOTR" + (x + 1)] != null && localStorage["SOTR" + (x + 1)] != "null") {
...
function scene_menu() {
    ...
    ctx.font = "20px NotoSans, sans-serif";
    ctx.fillText("Prototype 7", 660, 475);
    ...
    buttons.push([0, 800, 0, 500, () => {
        ...
        ctx.font = "12px NotoSans, sans-serif";
        ctx.fillText("Select a save to use.", 50, 200);
        ctx.fillText("The game autosaves at the current loaded save file,", 50, 224);
        ctx.fillText("this will be removed in the future!", 50, 236);


        loadsavebutton(0);
        let loadSave1 = new Button(0, 12, 96, 58, false, () => { saveNR = 1; loadGame(); changeScene(2); });
        loadSave1.render();

        loadsavebutton(1);
        let loadSave2 = new Button(0, 12, 96, 58, false, () => { saveNR = 2; loadGame(); changeScene(2); });
        loadSave2.render();

        loadsavebutton(2);
        let loadSave3 = new Button(0, 12, 96, 58, false, () => { saveNR = 3; loadGame(); changeScene(2); });
        loadSave3.render();
...
function scene_map() {
    ...
    sprite_pr += 32;
    if (sprite_pr == 64) { sprite_pr = 0; }

    switch (facing) {
        case "up":
            ctx.drawImage(sprites.bleu, sprite_pr, 96, 32, 32, 384, 256, 32, 32);
            break;
        case "left":
            ctx.drawImage(sprites.bleu, sprite_pr, 64, 32, 32, 384, 256, 32, 32);
            break;
        case "down":
            ctx.drawImage(sprites.bleu, sprite_pr, 0, 32, 32, 384, 256, 32, 32);
            break;
        case "right":
            ctx.drawImage(sprites.bleu, sprite_pr, 32, 32, 32, 384, 256, 32, 32);
            break;
    }
    switch (evilfacing) {
        case "up":
