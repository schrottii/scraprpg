        ...
        return {
            ...
            render(ctx) {
                ...
                ctx.drawImage(images[this.skin],
                    ...
                    ((zoom * scale) * (tileX + (CAM_OX == 0 ? kofs[0] * kofs[2] : 0) - this.kofs[0] * (this.kofs[2] / this.walkingSpeed) - xAdjust)) - ((zoom - 1) * scale * (width / 2)),
                    (zoom * scale) * (tileY + (CAM_OY == 0 ? kofs[1] * kofs[2] : 0) - this.kofs[1] * (this.kofs[2] / this.walkingSpeed) - Math.max(CAMERA_LOCK_Y, game.position[1] - 7.5) - ((zoom - 1) * scale * 7)),
                    zswm, zswm)
                if (this.talk == true) {
                    ctx.drawImage(images.talk,
                        ...
                        (zoom * scale) * (tileY - 1 + (CAM_OY == 0 ? kofs[1] * kofs[2] : 0) - this.kofs[1] * (this.kofs[2] / this.walkingSpeed) - Math.max(CAMERA_LOCK_Y, (game.position[1] - 7.5) - ((zoom - 1) * scale * 7))),
                        zswm, zswm)
                }
            },
