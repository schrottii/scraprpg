...
class WGGJ_Square extends WGGJ_Base {
    ...
    currentW() {
        return ~~(wggj.canvas.w * this.w + 0.5) + this.sizeOffset[0];
    }

    currentH() {
        return ~~(wggj.canvas.h * this.h + 0.5) + this.sizeOffset[1];
    }
...
class WGGJ_Image extends WGGJ_Base {
    ...
    currentX() {
        ...
        return (Pquadratic * this.x) - Pcentered + parentX + this.offset[0];
    }

    currentY() {
        let parentY = this.parent != undefined ? objects[this.parent].scrolledY : 0;

        return wggj.canvas.h * this.y + parentY + this.offset[1];
    }

    currentW() {
        let Pquadratic2 = this.quadratic ? wggj.canvas.h : wggj.canvas.w;

        return Pquadratic2 * this.w + this.sizeOffset[0];
    }

    currentH() {
        return wggj.canvas.h * this.h + this.sizeOffset[1];
    }
...
class WGGJ_Text extends WGGJ_Base {
    ...
    render(parented = false) {
        ...
        let renderY = wggj.canvas.h * this.y + this.offset[1];
...
function wggjAudioInit() {
    let audioPlayer = document.getElementById("wggjAudio");
    try {
        if (audio == undefined || audio.length < 1 || audioPlayer == undefined) return false;
    }
    catch {
        return false;
    }
    ...
    */
    wggj.audio.musicPlayer = audioPlayer;
